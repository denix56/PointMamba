...
class MixerModel(nn.Module):
    ...
    def forward(self, input_ids, pos, inference_params=None):
        ...
        return hidden_states.to(pos.dtype)


def reorder_func(logit_embed_layer, logit_head_layer, permuter, logits_pos, neighborhood, center, order, tau, group_size, order_index):
    logits, _, _ = logit_embed_layer(neighborhood, center, aux_residual=logits_pos, order=order, order_index=order_index)
    logits = logit_head_layer(logits)

    P = permuter(logits.squeeze(-1), tau)
    center = torch.bmm(P, center)
    neighborhood = torch.einsum('bij,bjgd->bigd', P, neighborhood)

    # g = -torch.empty_like(logits).exponential_().log()  # Gumbel
    # scores = (logits + tau * g)
    #
    # order = torch.argsort(scores, dim=1, descending=True)
    # center = torch.gather(center, dim=1, index=order.expand(-1, -1, 3)).contiguous()
    # neighborhood = torch.gather(neighborhood, dim=1, index=order.unsqueeze(2).expand(-1, -1, group_size, 3)).contiguous()

    return logits, P, center, neighborhood


def plakett_luce_policy(logits, order, A):
    logits = torch.bmm(order, logits)
    #logits = torch.gather(logits, dim=1, index=order).contiguous()
    return -A * torch.mean(torch.sum(logits - torch.logcumsumexp(logits.flip(1), dim=1).flip(1), dim=1))
...
class StochasticNeuralSortPermuter(nn.Module):
    ...
    def forward(self, z: torch.Tensor, tau: float) -> torch.Tensor:
        ...
        g = -torch.log(-torch.log(torch.rand_like(z) + 1e-20) + 1e-20)
        z_tilde = z + tau * g  # [B, N]

        # 2) build pairwise absolute-diff matrix
        #    shape [B, N, N]: |z_tilde_i - z_tilde_j|
        z1 = z_tilde.unsqueeze(2)  # [B, N, 1]
        z2 = z_tilde.unsqueeze(1)  # [B, 1, N]
        abs_diff = torch.abs(z1 - z2)

        # 3) compute “scores” for each position
        #    row i should be large where z_tilde is large *and* far above others
        one_N = torch.ones((N, 1), device=z.device, dtype=z.dtype)
        R = z1 @ one_N.T  # [B, N, N], each row i equals z_tilde

        # 4) softmax over (R - cumulative_abs_diff) / tau
        #    this is the NeuralSort formula per Grover et al.
        P_hat = F.softmax((R - abs_diff.cumsum(dim=2)) / self.tau_2, dim=-1)
...
class PointMambaScan(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.logit_embed = MaskMamba(config)

        self.logits_pos = nn.Parameter(torch.randn(1, self.num_group, self.trans_dim))
        self.logit_head = nn.Linear(self.trans_dim, 1)
    ...
    def forward(self, pts, gt=None, init_baseline = False, tau=0.0, vis: bool = False, vis_dir: str = 'clouds_visuals', epoch=None):
        ...
        # # reordering strategy
        center_forward, order_forward, _, group_input_tokens_forward, pos_forward = serialization_func(center, group_input_tokens, pos,
                                                                              'hilbert')
        center_backward, order_backward, _, group_input_tokens_backward, pos_backward = serialization_func(center, group_input_tokens, pos,
                                                                                'hilbert-trans')

        neighborhood_forward = neighborhood.flatten(0, 1)[order_forward].reshape(B, G, S, -1)
        #neighborhood_backward = neighborhood.flatten(0, 1)[order_backward].reshape(B, G, S, -1)

        if self.baseline is not None:
            logits_forward, order_forward, _, neighborhood_forward = (
                reorder_func(self.logit_embed, self.logit_head, self.permuter, self.logits_pos, neighborhood_forward, center_forward,
                             order_forward, tau, self.group_size, order_index=0))
            # logits_backward, order_backward, _, _ = (
            #     reorder_func(self.logit_embed, self.logit_head, self.logits_pos, neighborhood_backward, center_backward,
            #                  order_backward, tau, self.group_size, order_index=1))
            group_input_tokens_forward = torch.bmm(order_forward, group_input_tokens_forward)
            #group_input_tokens_forward = torch.gather(group_input_tokens_forward, dim=1, index=order_forward.expand(-1, -1, self.trans_dim)).contiguous()
            #group_input_tokens_backward = torch.gather(group_input_tokens_backward, dim=1, index=order_backward.expand(-1, -1, self.trans_dim)).contiguous()
            pos_forward = torch.bmm(order_forward, pos_forward)
            #pos_forward = torch.gather(pos_forward, dim=1, index=order_forward.expand(-1, -1, self.trans_dim)).contiguous()
            #pos_backward = torch.gather(pos_backward, dim=1, index=order_backward.expand(-1, -1, self.trans_dim)).contiguous()

        if vis:
            assert vis_dir is not None, "vis_dir must be provided when vis=True"
            # if epoch provided, nest under epoch subdirectory
            save_dir = vis_dir
            if epoch is not None:
                save_dir = os.path.join(vis_dir, f"epoch_{epoch}")
            os.makedirs(save_dir, exist_ok=True)

            for b in range(B):
                # (G*S, D) => (M*K, 3)
                pc = neighborhood_forward[b].reshape(G * S, -1).cpu().numpy()
                # group indices [0,0,..,1,1,..]
                group_idx = np.repeat(np.arange(G), S)

                # save as .npz
                fname = os.path.join(save_dir, f"sample_{b:03d}.npz")
                np.savez(fname, points=pc, group_idx=group_idx)

                # (optional) save as colored PLY if Open3D is installed
                try:
                    import open3d as o3d
                    from matplotlib import cm

                    colors = cm.viridis(group_idx / float(G - 1))[:, :3]
                    pcd = o3d.geometry.PointCloud()
                    pcd.points = o3d.utility.Vector3dVector(pc)
                    pcd.colors = o3d.utility.Vector3dVector(colors)
                    ply_name = os.path.join(save_dir, f"sample_{b:03d}.ply")
        ...
        if gt is not None:
            loss1, acc = self.get_loss_acc(ret, gt)
            ...
            if self.baseline is not None:
                ...
                loss_policy = plakett_luce_policy(logits_forward, order_forward, A) #+ plakett_luce_policy(logits_backward, order_backward, A)
            elif init_baseline:
                self.baseline = reward

            loss_full = loss1 + loss_policy
            return ret, loss_full, loss1, loss_policy, acc
        else:
            return ret
...
class MaskMamba(nn.Module):
    ...
    def forward(self, neighborhood, center, order, order_index, aux_residual=None, noaug=False):  # generate mask
...
class Point_MAE_Mamba_serializationV2(nn.Module):
    def __init__(self, config):
        ...
        config_emb = copy.copy(config)
        config_emb.mamba_config.mask_ratio = 0
        config_emb.mamba_config.depth = 1
        self.logit_embed = MaskMamba(config_emb)
    ...
    def forward(self, pts, vis=False, tau=0.0, init_baseline = False, **kwargs):
        neighborhood, center = self.group_divider(pts)
        B, G, S, _ = neighborhood.shape
        ...
        if self.baseline is not None:
            #
            logits, _, _ = self.logit_embed(neighborhood, center, aux_residual=self.logits_pos, order=order, order_index=order_index[0])
            logits = self.logit_head(logits)
            ...
            order = torch.argsort(scores, dim=1, descending=True)
            center = torch.gather(center, dim=1, index=order.expand(-1, -1, 3)).contiguous()
            neighborhood = torch.gather(neighborhood, dim=1, index=order.unsqueeze(2).expand(-1, -1, self.group_size, 3)).contiguous()
        ...
        x_full = torch.zeros_like(group_input_tokens)
        x_full[~mask] = x_vis.reshape(-1, C)
        x_full[mask] = self.mask_token.reshape(-1, C).to(x_full.dtype)
        pos_full = torch.zeros_like(group_input_tokens)
        ...
        if self.baseline is not None:
            ...
            A = reward - self.baseline
            logits = torch.gather(logits, dim=1, index=order).contiguous()
            loss_policy = -torch.mean(A * torch.sum(logits - torch.logcumsumexp(logits.flip(1), dim=1).flip(1), dim=1))
        elif init_baseline:
            self.baseline = reward
        ...
        if vis:  # visualization
            ...
            return loss_full, loss1, loss_policy
